...
def updtMedication(treatmentId, speechText):
    ...
    zenbo.robot.set_expression(RobotFace.DEFAULT, (response + '. Did you manage to take this medication?...'), {'speed':zenbo_speakSpeed, 'pitch':zenbo_speakPitch, 'languageId':zenbo_speakLanguage} , sync = True)    
    ...
    body = {'treatmentId' : "'"+str(treatmentId)+"'", 'user' : '', 'medication' : '', 'data_inicio' : '', 'data_fim' : '', 'last_occurrence' : '', 'medic' : '', 'descricao' : '', 'monitorado' : 's', 'atendido' : "'"+str(respostaAtendido)+"'"}
    
    response = requests.put(api_endpoint + request_endpoint, data = body)
    response = response.json()
...
updtMedication(treatmentId, response)
time.sleep(int(1))  

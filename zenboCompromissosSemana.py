...
api_endpoint = "http://192.168.7.15:1515"
userId = sys.argv[1]
...
def getAppointments(userId):
    ...
    request_endpoint = "/api/data/appointment/week?id=" + str(userId) 
...
def formatHora(horario):
    ...
    p = inflect.engine()
    hora = p.number_to_words(int(hora))
    minuto = p.number_to_words(int(minuto))
...
zenbo.robot.set_expression(RobotFace.DEFAULT, 'Lets check if there are appointments within the next 7 days...', {'speed':zenbo_speakSpeed, 'pitch':zenbo_speakPitch, 'languageId':zenbo_speakLanguage} , sync = True)
time.sleep(int(1))
...
if response == []:
    ...
    zenbo.robot.set_expression(RobotFace.DEFAULT, '...well, guess there are no appointments this week', {'speed':zenbo_speakSpeed, 'pitch':zenbo_speakPitch, 'languageId':zenbo_speakLanguage} , sync = True)
    time.sleep(int(1))
    
else:
    
    zenbo.robot.set_expression(RobotFace.DEFAULT, '...Guess You have appointments for this week. Listing them now...', {'speed':zenbo_speakSpeed, 'pitch':zenbo_speakPitch, 'languageId':zenbo_speakLanguage} , sync = True)
    time.sleep(int(1))
